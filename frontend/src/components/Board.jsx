...
const Board = forwardRef(({ opening, initialFen = null, mode = 'play', puzzleSolution = null, savedGame = null }, ref) => {
  ...
  useEffect(() => {
    // load a saved game for stepping
    if (!savedGame) return
    const game = new Chess()
    try {
      const moves = typeof savedGame.moves === 'string' ? JSON.parse(savedGame.moves) : savedGame.moves
      ...
      setStatus(`Loaded game: ${savedGame.title || 'Untitled Game'}`)
      setMoveIndex(moves.length - 1)
      setOpeningMoves(moves)
    } catch (e) {
      console.error('Failed to load saved game moves', e)
      setStatus('Failed to load saved game')
    }
  }, [savedGame])
  ...
  return (
    <div className="panel board-panel" ref={containerRef}>
      <div className="panel-header">
        <strong>Chess Board</strong>
        <div className="board-toolbar">
          <button className="btn btn-sm btn-outline-secondary" onClick={reset}>Reset</button>
          <button className="btn btn-sm btn-outline-secondary" onClick={undo}>Undo</button>
          <button className="btn btn-sm btn-outline-secondary" onClick={redo}>Redo</button>
          <button className="btn btn-sm btn-outline-secondary" onClick={exportPgn}>Export PGN</button>
          <button className="btn btn-sm btn-accent" onClick={toggleOrientation}>Flip</button>
        </div>
      </div>
      ...
      <div className="d-flex flex-column flex-md-row gap-3">
        ...
        <div style={{ minWidth: 200, flex: 1 }}>
          ...
          <h6 className="panel-title" style={{ fontSize: '0.95rem' }}>Moves</h6>
          ...
          <div className="mt-3 text-muted small">
            ...
            {status && <div className="status-bar mt-2">{status}</div>}
          </div>
        </div>
      </div>
    </div>
