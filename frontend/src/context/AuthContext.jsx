...
export function AuthProvider({ children }) {
  ...
  async function addFavorite(openingId) {
    ...
    const r = await apiClient.post('/api/user/me/favorites', { openingId })
    if (r.ok) {
      // update user object
  ...
  async function removeFavorite(openingId) {
    if (!token) return { ok: false, error: 'not authenticated' }
    const r = await apiClient.delete(`/api/user/me/favorites/${openingId}`)
  ...
  async function updateProfile(data) {
    if (!token) return { ok: false, error: 'not authenticated' }
    const r = await apiClient.put('/api/user/me', data)
  ...
  async function getProfile() {
    if (!token) return { ok: false, error: 'not authenticated' }
    const r = await apiClient.get('/api/user/me')
    if (r.ok) return { ok: true, data: r.body }
    return { ok: false, error: r.body }
  }
